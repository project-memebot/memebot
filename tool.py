...
async def wait_buttons(msg, memeid, bot):
    interaction = await bot.wait_for(
        "button_click", check=lambda i: i.component.label == "🚨 신고하기"
    )
    await interaction.respond(
        content="신고 사유를 선택해 주세요",
        components=[
            Select(
                placeholder="신고 사유",
                options=[
                    SelectOption(
                        label="1", value="1", description="대한민국 법에 어긋나는 짤(초상권, etc...)"
                    ),
                    SelectOption(
                        label="2",
                        value="2",
                    ...
                    SelectOption(
                        label="4", value="4", description="정치드립/19금/19금 드립 등 불쾌할 수 있는 짤"
                    ),
                    SelectOption(
                        label="5",
                        value="5",
    ...
    date = __import__("datetime").datetime.utcnow() + __import__("datetime").timedelta(
        hours=9
    )
